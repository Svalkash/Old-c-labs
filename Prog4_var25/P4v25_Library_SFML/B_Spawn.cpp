...
namespace P4v25_SFML
{
	...
	double B_Spawn::remainingTime() const
	{
		if (spawnList.size() == 0)
			return 0;
		//changed because iterator must be const
		multimap<double, Enemy*>::const_iterator next = spawnList.begin(); //if size == 0, returns begin()
		for (multimap<double, Enemy*>::const_iterator it = spawnList.begin(); it != spawnList.end(); ++it)
			if ((*it).first < (*next).first)
				next = it;
		return fmax((*next).first - timeW, 0);
	}
	...
	{
		spawnList.insert(make_pair(t, pe)); //that works?
	}
	...
	std::string B_Spawn::getInfo(const Level &L) const
	{
		stringstream s;
		s << setprecision(PRINT_PRECISION) << fixed;
		s << endl;
		s << "Enemies left: " << spawnList.size();
		if (spawnList.size() > 0)
			s << endl << "Next enemy after: " << remainingTime() << " s";
		return s.str();
	}
}