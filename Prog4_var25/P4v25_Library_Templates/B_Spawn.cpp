...
namespace P4v25_T
{
	...
	{
		for (myMultimap<double, Enemy*>::iterator it = spawnList.begin(); it != spawnList.end(); ++it)
			if ((*it).first <= timeW)
				return 1;
		return 0;
	}

	myMultimap<double, Enemy*>::iterator B_Spawn::findNext()
	{
		myMultimap<double, Enemy*>::iterator next = spawnList.begin(); //if size == 0, returns begin()
		for (myMultimap<double, Enemy*>::iterator it = spawnList.begin(); it != spawnList.end(); ++it)
	...
	int B_Spawn::releaseF(Level &L)
	{
		if (spawnList.size() == 0)
			return 0;
		myMultimap<double, Enemy*>::iterator next = findNext();
	...
	{
		spawnList.insert(myPair<const double, Enemy*>(t, pe)); //that works?
	}
	...
	ostream &operator <<(ostream &f, B_Spawn &bs)
	{
		...
		//writing enemy list
		for (myMultimap<double, Enemy*>::iterator it = bs.spawnList.begin(); it != bs.spawnList.end(); ++it)
		{
			f << endl;
			f << *(*it).second << endl;
			f << (*it).first;
	...
	B_Spawn::~B_Spawn()
	{
		//myMultimap<double, Enemy*> can't delete pointers, let's help it!
		for (myMultimap<double, Enemy*>::iterator it = spawnList.begin(); it != spawnList.end(); ++it)
			delete (*it).second;
	}
